    ...
    applyFilters() {
        this.filteredProfiles = this.profiles.filter(profile => {
            ...
            if (this.currentFilters.personality !== 'all') {
                ...
                if (targetPersonality === 'cheerful') hasPersonality = personality.some(p => p && (p.includes('발랄함') || p.includes('활발함')));
                if (targetPersonality === 'warm') hasPersonality = personality.some(p => p && p.includes('따뜻함'));
                if (targetPersonality === 'intellectual') hasPersonality = personality.some(p => p && p.includes('지적임'));
                if (targetPersonality === 'humorous') hasPersonality = personality.some(p => p && p.includes('유머러스'));
...
function viewProfile(id) {
    ...
    window.location.href = `detail_new.html?id=${id}`;
}
